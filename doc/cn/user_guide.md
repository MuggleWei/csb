...
* --art-dir: 制品搜索目录, 默认情况下 `hpb` 会读取[默认配置文件](#默认配置文件), 并将文件中的 `artifacts/path` 依次加入制品搜索目录中
* -p, --param: 设置构建参数
* -o, --output-dir: 指定输出目录, 若没有设置, 默认情况下使用 `任务目录/output` 作为输出目录

### hpb build - hello world
让我们通过一个简单的例子来展示 `hpb build` 的使用  
...
```
2023-04-07 22:38:11,769|root|INFO|builder.py:54 - hpb builder run task hello.20230407-223811-748754
...
同时, 当前目录下还多了一个 `_hpb` 目录, 在其中可以找到和上面任务同名的子目录, 这是 `hpb` 默认的用于执行任务生成的文件夹
    ...
    steps:
      - name: step1
        run: >
          ...
          echo "${bar}";
```
我们在文件中增加了 `varables`, 并定义了变量 `foo`, 将其值设置为 `foo`. 此时, 我们直接运行 `hpb build -c echo.yml` 将会看到错误日志
```
...
`hpb build -c echo.yml -p bar="hello bar"`  
    ...
    steps:
      - name: step1
        run: >
          ...
          echo "$(pwd)";
```
运行 `hpb build -c inner_var.yml`, 我们将可以看到被打印出的目录信息, 以及当前目录(**注意, 这个例子中使用了 unix/linux 的命令 pwd, 所以在 windows 上直接运行是会报错的**)  

除了上面看到的两个内建变量, 还有许多内建变量, 它们都由 `HPB_` 开头, 具体的见下表
| 名称 | 描述 |
| ---- | ---- |
| HPB_ROOT_DIR | workflow 初始的工作目录 |
| HPB_TASK_DIR | 为本任务分配的目录, 用于日志输出, workflow 命令执行流输出等等 |
| HPB_OUTPUT_DIR | 建议使用的输出目录, 即 `hpb build -o` 所指定的目录 |
| HPB_FILE_DIR | 配置文件所在的目录 |
| HPB_FILE_NAME | 配置文件名称(不包含路径) |
| HPB_FILE_PATH | 配置文件路径 |
| HPB_TASK_NAME | 任务名称, 即 `hpb build --task-name` 所指定的名称 |
| HPB_TASK_ID | 任务 id, 即 `hpb build --task-id` 所指定的 id |
| HPB_GIT_REF | 若 HPB_ROOT_DIR 是 git 工程中的目录, 那么 HPB_GIT_REF 会按一下优先级被指定: git tag > git commit_id |
| HPB_GIT_TAG | 若 HPB_ROOT_DIR 是 git 工程中的目录, HPB_GIT_TAG 会被设置为当前的 git tag |
| HPB_GIT_COMMIT_ID | 若 HPB_ROOT_DIR 是 git 工程中的目录, HPB_GIT_COMMIT_ID 会被设置为当前的 git commit id |
| HPB_GIT_BRANCH | 若 HPB_ROOT_DIR 是 git 工程中的目录, HPB_GIT_BRANCH 会被设置为当前的 git branch 名称 |
